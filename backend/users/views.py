...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(f"User logged in: {user.username} (Role: {user.role})", extra={
            'tags': {
                'action': 'login', 
                'status': 'success', 
                'user': user.username,
...
class CustomTokenRefreshView(TokenRefreshView):
    ...
    def post(self, request, *args, **kwargs):
        refresh_token = request.COOKIES.get("refresh_token")
        if not refresh_token:
            # logger.info("Token refresh failed: No refresh token in cookies")
            return Response({"detail": "Refresh token not found"}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        try:
            ...
            # logger.warning(f"Token refresh failed: Invalid or expired token", extra={
            #     'tags': {'action': 'token_refresh', 'status': 'expired'}
            # })
    ...
    def post(self, request):
        ...
        if refresh_token:
            try:
                token = RefreshToken(refresh_token)
                token.blacklist()
                logger.info(f"User logged out: {user_name}", extra={
                    'tags': {'action': 'logout', 'user': user_name}
                })
            except Exception as e:

                logger.error(f"Logout error for {user_name}: {str(e)}", exc_info=True)
...
def register_with_invite(request, code):
    ...
    try:
        ...
        logger.warning(f"Invalid invite code attempt: {code}")
        return Response(
    ...
    if invite.used:
        logger.info(f"Used invite link accessed: {code}")
        return Response(
    ...
    if timezone.now() > invite.created_at + timedelta(hours=24):
        logger.info(f"Expired invite link accessed: {code}")
        return Response(
    ...
    try:
        ...
        logger.error(f"Invite exists but User 1C not found: {bin_to_guid_1c(invite.user_id_1C)}")
        return Response(
    ...
    try:
        ...
        logger.info(f"User registration completed via invite", extra={
            'tags': {
        ...
        logger.error(f"Critical error during registration saving: {str(e)}", exc_info=True)
...
def get_customers(request):
    ...
    duration = time.time() - start_time
    logger.info(f"Manager {user_name} requested customer list", extra={
        'tags': {
...
def get_user_name_view(request):
    ...
    duration = time.time() - start_time
    logger.info(f"User profile name requested: {user.username}", extra={
        'tags': {
...
def change_password_client(request):
    ...
    if not user.check_password(old_password):
        logger.warning(f"Failed password change attempt: Incorrect old password for user {user.username}", extra={
            'tags': {'action': 'change_password', 'status': 'failed', 'user': user.username}
        })
        return Response(
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.info(f"Password changed successfully for user: {user.username}", extra={
            'tags': {
        ...
        logger.error(f"Critical error during password change for {user.username}: {str(e)}", exc_info=True)
        return Response(
            {"error": f"Помилка при збереженні нового пароля."},
...
def admin_change_user_password(request, user_id):
    ...
    try:
        ...
        logger.error(f"Error in admin_change_user_password: {str(e)}", exc_info=True)
...
def get_all_users_view(request):
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.info(f"ADMIN {admin_user.username} fetched all users list", extra={
            'tags': {
...
def admin_edit_user_view(request, user_id):
    ...
    duration = time.time() - start_time
    logger.info(f"ADMIN {admin_user.username} edited user {user.username}", extra={
        'tags': {
...
def get_current_user(request):
    ...
    logger.debug(f"Session heartbeat for {user.username}", extra={
        'tags': {'action': 'get_current_user', 'user': user.username, 'role': user.role}
    })
...
def get_dealer_addresses_change(request):
    ...
    try:
        ...
        logger.warning(f"Unauthorized address access: {str(e)}")
        return Response({"error": str(e)}, status=403)

    # 📦 SQL
    logger.info(f"Fetching parsed addresses for contractor", extra={
        'tags': {'action': 'get_parsed_addresses', 'contractor': contractor_guid}
    })
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.info(f"Parsed addresses retrieved successfully", extra={
            'tags': {
                'action': 'get_parsed_addresses',
        ...
        logger.error(f"SQL Error in GetDealerAddressesParsed: {str(e)}", exc_info=True)
        return Response({"error": "Failed to retrieve parsed addresses"}, status=500)
...
def create_api_key(request):
    ...
    try:
        ...
        duration = time.time() - start_time
        # Аудит: Хто створив ключ і для кого. САМ КЛЮЧ НЕ ЛОГУЄМО.
        logger.info(f"ADMIN {admin_user.username} created API key for dealer {dealer.username}", extra={
            'tags': {
...
def list_user_api_keys(request, user_id):
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.info(f"ADMIN {admin_name} viewed API keys for user_id {user_id}", extra={
            'tags': {
        ...
        logger.error(f"Error listing API keys for user_id {user_id}: {str(e)}", exc_info=True)
        return Response({"detail": "Internal server error"}, status=500)
...
def deactivate_api_key(request, key_id):
    ...
    try:
        ...
        if not key.is_active:
            return Response({"detail": "API key already inactive"}, status=400)

        key.is_active = False
        key.save(update_fields=["is_active"])

        duration = time.time() - start_time
        logger.info(f"ADMIN {admin_name} deactivated API key ID {key_id} (Dealer: {key.user.username})", extra={
            'tags': {
                'action': 'deactivate_api_key',
                'admin': admin_name,
                'target_dealer': key.user.username,
                'duration_sec': round(duration, 3)
            }
        })

        return Response({"detail": "API key deactivated"}, status=200)

    except UserApiKey.DoesNotExist:
        return Response({"detail": "API key not found"}, status=404)
    except Exception as e:
        logger.error(f"Error deactivating API key {key_id}: {str(e)}", exc_info=True)
        return Response({"detail": "Internal server error"}, status=500)
...
def save_dealer_address_coords(request):
    ...
    logger.info(f"User {user.username} is saving address coordinates", extra={
        'tags': {
            'action': 'save_address_coords',
            'user': user.username,
            'contractor': contractor_guid
        }
    })
    ...
    for f in required_fields:
        if f not in data or data[f] in [None, ""]:
            logger.warning(f"Address validation failed: missing field '{f}' for user {user.username}")
    ...
    try:
        ...
        # Тут зазвичай йде відправка в 1С (send_to_1c)
        # Якщо ви підключите send_to_1c, не забудьте залогувати її результат
        
        duration = time.time() - start_time
        logger.info(f"Address for 1C prepared successfully", extra={
            'tags': {
                'action': 'save_address_coords',
                'contractor': contractor_guid,
        ...
        logger.error(f"Error preparing address for 1C: {str(e)}", exc_info=True, extra={
            'tags': {'action': 'save_address_coords', 'user': user.username}
        })
        return Response({"error": "Внутрішня помилка при обробці адреси"}, status=500)
...
class CreateInvitationView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if user:
                ...
                if user.is_active:
                    logger.info(f"Invite failed: User {user.username} already active", extra={
                        'tags': {'action': 'create_invite', 'status': 'already_active', 'user': user.username}
                    })
                    return Response(
                ...
                if last_invite and not last_invite.used:
                    ...
                    if now < expiration_threshold:
                        ...
                        logger.info(f"Invite throttle: Active invite exists for {user.username}")
            ...
            duration = time.time() - start_time
            logger.info(f"Invitation created for {user.username}", extra={
                'tags': {
            ...
            logger.error(f"Critical error in CreateInvitationView: {str(e)}", exc_info=True)
            return Response(
...
class CreateAdminDirectView(APIView):
    ...
    def post(self, request):
        ...
        start_time = time.time()
        admin_creator = request.user.username
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.info(f"ADMIN {admin_creator} created NEW ADMIN: {username}", extra={
                'tags': {'action': 'create_admin_direct', 'creator': admin_creator, 'new_admin': username}
            })
            ...
            logger.error(f"Failed to create admin {username}: {str(e)}", exc_info=True)
...
def get_active_users_1c(request):
    ...
    if request.method == "GET":
        try:
            ...
            duration = time.time() - start_time
            logger.info(f"GetActiveUsers1С executed", extra={
                'tags': {'action': 'sql_get_active_users', 'count': len(results), 'duration_sec': round(duration, 3)}
            })

            return JsonResponse({"users": results}, safe=False)

        except Exception as e:
            logger.error(f"SQL Error in get_active_users_1c: {str(e)}", exc_info=True)
...
def get_telegram_link(request):
    ...
    # logger.debug(f"User {user.username} requested Telegram bot link", extra={
    #     'tags': {'action': 'get_tg_link_request', 'user': user.username}
    # })
    
    # Перевіряємо, чи є у користувача ID з 1С (або інше поле, яке ви використовуєте)
    if not user.user_id_1C:
        logger.warning(f"Telegram link generation failed: User {user.username} has no 1C ID", extra={
            'tags': {'action': 'get_tg_link', 'status': 'failed', 'reason': 'no_1c_id'}
        })
        return Response(
    ...
    try:
        ...
        # 4. Фінальний лог успіху
        logger.info(f"Telegram link generated successfully for {user.username}", extra={
            'tags': {
                'action': 'get_tg_link',
                'status': 'success',
                'user': user.username,
        ...
        logger.error(f"Critical error generating Telegram link for {user.username}: {str(e)}", exc_info=True, extra={
            'tags': {'action': 'get_tg_link', 'status': 'error'}
        })
        return Response({"error": "Internal server error"}, status=500)
...
export default function CreateCustomerBillModal({
  ...
  // Обробка Escape та блокування скролу
  useEffect(() => {
    if (!isOpen) return;
    const handleEsc = (event) => {
      if (event.key === "Escape") onClose();
  ...
  // Завантаження профілю та скидання форми при відкритті модалки
  useEffect(() => {
    if (!isOpen) return;
    ...
    setDeliveryDate("");
    setInternalComment("");
    setOrderItems([{ itemGUID: "", quantity: 1, price: 0, width: "", height: "" }]);
    ...
    <div className="bill-modal-overlay">
      <div className="bill-modal-window">
        <div className="bill-modal-header">
          ...
          <button className="bill-close-btn" onClick={onClose} disabled={loading}>
            ✕
          </button>
        </div>
        ...
        <div className="bill-form-scroll">
          {fetchingProfile ? (
            <div className="flex justify-center items-center p-8 text-gray-500 gap-2">
              <FaSpinner className="animate-spin" /> {t("common.loading", "Завантаження...")}
            </div>
          ) : (
            <div className="bill-form">
              {step === STEPS.BASE && (
                    ...
                    <div key={idx} className="series-list-product">
                      ...
                      <div className="bill-field full">
                        ...
                        <BillItemSelect
                          ...
                          items={itemsList}
                          placeholder={t("create_bill.placeholders.product")}
                          onChange={(val) => handleItemChange(idx, "itemGUID", val)}
                        />
                      </div>

...
  // Додаємо в компонент PortalOriginal
const handleDeleteCalculation = (calcId) => {
  // Видаляємо прорахунок з state
...
        const params = { year: selectedYear };
        if (dealer && dealer.id) {
        params.customer_id = dealer.id;
      } else {
        params.customer_id = null; // або можна просто не передавати це поле
      }
...
            const totalAmount = orders.filter(o => o.status !== 'Відмова').reduce((sum, o) => sum + (o.amount || 0), 0);
            const totalPaid = orders.filter(o => o.status !== 'Відмова').reduce((sum, o) => sum + (o.paid || 0), 0);

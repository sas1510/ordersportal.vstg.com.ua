...
const MobilePaymentsView = ({
  ...
  const getChannelBadge = (channel) => {
    const map = {
      bank: t('payments.channels.bank'),
      cash: t('payments.channels.cash'),
      order: t('payments.channels.order_short'),
      none: "—",
    };
    return (
      <span className={`mobile-channel-badge ${channel}`}>
  ...
  return (
    <div className="mobile-payments-container">
      ...
      <div className="mobile-groups-list">
        {groups.length === 0 && (
          <div className="mobile-empty">{t('common.no_data')}</div>
        )}
            ...
            <div className="mobile-day-surface">
              ...
              <div className="mobile-contracts-list">
                {Object.values(group.initialContracts).map((c, idx) => (
                  <div key={idx} className="mobile-contract-badge">
                    <span className="mobile-contract-name">
                      {/* {t(`backend.${c.contractName}`, c.contractName)}: */}
                      <AutoTranslatedText text={c.contractName} /> :
                    </span>
                    <span className="mobile-contract-value">
                ...
                return (
                  <div
                    ...
                    className={`mobile-doc-card ${
                      firstItem.FlowDirection === "Прихід"
                        ? "income"
                        : firstItem.FlowDirection === "Витрата" || firstItem.FlowDirection === "Розхід"
                          ? "expense"
                          : "neutral"
                    }`}
                  >
                    ...
                    >
                      ...
                      <div className="mobile-doc-operation">
                          {firstItem.DocumentType === "КорректировкаДолга"
                          ? <AutoTranslatedText text={`Коригування. ${firstItem.DescriptionCor || ""}`} />
                          : <AutoTranslatedText text={firstItem.DealType || firstItem.DocumentType || "—"} />}
                      </div>

                      <div className="mobile-doc-amounts">
                        <div className="mobile-amount-item">
                          <span className="mobile-amount-label">{t('payments.start_short', 'Початок')}:</span>
                          <span className="mobile-amount-value">
                            {formatCurrency(docGroup.CumSaldoStart, currency, i18n)}
                          </span>
                        </div>
                        ...
                        <div className="mobile-amount-item mobile-amount-total">
                          ...
                          <span className="mobile-amount-value text-bold">
                            {formatCurrency(docGroup.lastCumSaldo, currency, i18n)}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="mobile-expand-btn">
                          {isExpanded ? (
                            <>
                              ...
                              {t('payments.hide_orders_mobile')}
                            </>
                          ) : (
                            <>
                              <i className="fa-solid fa-chevron-down" style={{ marginRight: 6 }} />
                              {t('payments.show_orders_mobile')}
                            </>
                          )}
                        </div>
                      ) : (
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="order-mini-grid">
                                ...
                                <div className="order-mini-col">
                                  <span className="order-mini-label">{t('payments.percent', 'Відсоток оплати')}</span>
                                  <span className="order-mini-value">
      ...
      {showScrollTop && (
        <button
          className="mobile-scroll-top"
          onClick={scrollToTop}
          aria-label={t('common.to_top', 'На верх')}
        >
          <i className="fa-solid fa-chevron-up" />
        </button>
      )}

...
const useIsMobile = () => {
  const [isMobile, setIsMobile] = useState(window.matchMedia("(max-width: 1260px)").matches);
  useEffect(() => {
...
export default function PaymentsPage() {
  ...
  const contractorGUID = JSON.parse(localStorage.getItem("user") || "{}")?.user_id_1c || localStorage.getItem("contractor_guid");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [resPage, resDebts] = await Promise.all([
        axiosInstance.get("/payments/get_dealer_payment_page_data/", { params: { contractor: contractorGUID } }),
        axiosInstance.get("/partner-debts/", { params: { contractor_guid: contractorGUID } })
      ]);
  ...
  const contractFilters = useMemo(() => {
    const map = new Map();
    orders.forEach((o) => {
      if (o.Dogovor_GUID && o.DogovorName) map.set(o.Dogovor_GUID, o.DogovorName);
    });
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((o) => {
      ...
      const contractOk = contractFilter === "all" || o.Dogovor_GUID === contractFilter;
      const searchOk = !search || (o.OrderNumber || "").toString().toLowerCase().includes(search.toLowerCase());
  ...
  const handlePayFromDetails = (zakazNum) => {
    const orderToPay = orders.find(o => String(o.OrderNumber) === String(zakazNum));
    ...
    setDetailModalOpen(false);
    addNotification(`Замовлення № ${zakazNum} додано у фільтр пошуку.`, "success");
  ...
  const Sidebar = (
    <div className={`content-filter-payment column ${isMobile ? (isSidebarOpen ? "open" : "closed") : ""}`}>
      ...
      <div className="search-wrapper-payment">
        ...
        {!!search && <span className="icon icon-cancel2 clear-search-payment" onClick={() => setSearch("")} />}
      </div>
      ...
      <ul className="filter column align-center">
        {STATUS_FILTERS.map(({ key, label, icon }) => {
          ...
          return (
            <li key={key} className={`filter-item ${statusFilter === key ? "active" : ""} ${count === 0 ? "empty-filter" : ""}`}
                onClick={() => { if (count > 0) setStatusFilter(key); if (isMobile) setIsSidebarOpen(false); }}>
      ...
      <ul className="filter column align-center">
        <li className={`filter-item ${contractFilter === "all" ? "active" : ""}`} 
            onClick={() => { setContractFilter("all"); if (isMobile) setIsSidebarOpen(false); }}>
          <span className="icon icon-files-empty font-size-24" />
          <span className="w-100">Усі договори</span>
          <span>{orders.length}</span>
        </li>
        {contractFilters.map((c) => (
          <li key={c.guid} className={`filter-item ${contractFilter === c.guid ? "active" : ""}`} 
              onClick={() => { setContractFilter(c.guid); if (isMobile) setIsSidebarOpen(false); }}>
            <span className="icon icon-file-text2 font-size-24" />
            <span className="w-100 no-wrap-ellipsis" title={c.name}>{c.name}</span>
            <span>{c.count}</span>
          </li>
        ))}
      </ul>
          ...
          <div className="pp-header">
            <div className="pp-title-header row ai-center gap-7">
              {isMobile && !isSidebarOpen && (
                <span className="icon icon-menu font-size-24" onClick={() => setIsSidebarOpen(true)} style={{cursor:'pointer'}}/>
              )}
              <span>Фінанси</span>
            </div>
            <button className="pp-reload" onClick={loadData}>⟳ Оновити дані</button>
          </div>
  ...
  <div className="analytics-container">
    ...
    <div className="analytics-mobile-grid mobile-only-payment">
      <div className="analytics-card">
        ...
        <span className="value bold-text">{debtTotal.CustomerLimit ? `${formatCurrency(debtTotal.CustomerLimit)} ₴` : "—"}</span>
      </div>
      ...
      <div className="analytics-card clickable" onClick={() => Number(debtTotal.BezPeredOplaty || 0) > 0 && showDebtDetails('no_prepayment')}>
        <span className="label">Без передоплати</span>
        <span className="value orange-text">{formatCurrency(debtTotal.BezPeredOplaty)} ₴</span>
      </div>

      <div className="analytics-card clickable" onClick={() => Number(debtTotal.DebtMoreTen || 0) > 0 && showDebtDetails('critical')}>
        <span className="label">Борг {'>'} 10дн</span>
        <span className="value red-text">{formatCurrency(debtTotal.DebtMoreTen)} ₴</span>
      </div>

      <div className="analytics-card clickable" onClick={() => Number(debtTotal.Debt || 0) > 0 && showDebtDetails('in_route')}>
        <span className="label">У маршрутах</span>
        <span className="value dark-orange-text">{formatCurrency(debtTotal.Debt)} ₴</span>
      </div>

      <div className="analytics-card">
        <span className="label">Борг (реаліз.)</span>
        <span className="value dark-orange-text">{formatCurrency(Number(debtTotal.Debt || 0) + Number(debtTotal.Summa || 0))} ₴</span>
      </div>
    </div>
  </div>
)}
          ...
          <div className="pp-badges">
            {contracts.length === 0 ? <div className="pp-empty">Аванси відсутні</div> : 
              contracts.map((c, i) => (
          ...
          <div className="pp-orders-wrapper">
             {filteredOrders.length === 0 ? <div className="pp-empty">Немає замовлень за обраними фільтрами</div> : (
                ...
                <div className="pp-order-card" key={i}>
                  <div className="pp-row pp-order-row">
                    ...
                    <div className="pp-status-col">
                      <span className={`status-pill ${STATUS_COLORS[normalizeStatus(o.OrderStage)] || "status-unknown"}`}>
                        {normalizeStatus(o.OrderStage)}
                      </span>
                    </div>
                    <div className="pp-info"><span>Сума:</span> <strong className="order-sum">{formatCurrency(o.OrderSum)}</strong></div>
                    <div className="pp-info"><span>Оплачено:</span> <strong className="pp-green">{formatCurrency(o.PaidAmount)}</strong></div>
                    <div className="pp-info"><span>Залишок:</span> <strong className="pp-red">{formatCurrency(o.DebtAmount)}</strong></div>
                    <div className="pp-pay-btn-wrapper"><button className="pp-pay-btn" onClick={() => openPaymentModal(o)}>Оплатити</button></div>
                  </div>
                </div>
               ))
             )}
        ...
        <div className="pay-modal-overlay" onClick={() => setDetailModalOpen(false)}>
          <div className="pay-modal-window" style={{width: '750px'}} onClick={e => e.stopPropagation()}>
              ...
              <table className="details-list-table">
                ...
                <tbody>
                  {filteredDetailOrders.length === 0 ? (
                    <tr><td colSpan="5" style={{textAlign: 'center', padding: '20px'}}>Дані відсутні</td></tr>
                  ) : (
                    filteredDetailOrders.map((o, idx) => (
                      <tr key={idx}>
                        <td className="bold">{o.ZakazNum}</td>
                        <td className="mobile-none">{formatDateHumanShorter(o.ZakazDate?.slice(0, 10))}</td>
                        <td className="bold" style={{color: '#1da8df'}}>
                          {formatCurrency(o.Debt || o.BezPeredOplaty || o.NedoAvans || o.Summa)} грн
                        </td>
                      ...
                      <td className="detail-action">
                          <div className="center-wrapper">
                            ...
                            >
                              ...
                              <span className="btn-text mobile-none"> Знайти</span>
                            </button>
                          </div>
                        </td>
            ...
            <div className="pay-modal-footer">
              <button className="pay-btn-cancel" onClick={() => setDetailModalOpen(false)}>Закрити</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalOpen && selectedOrder && (
        <PaymentModal order={selectedOrder} contracts={contracts} onClose={closeModal} onConfirm={makePayment} formatCurrency={formatCurrency} />
      )}
    </div>
  );
}
...
const FilesPage = () => {
    ...
    const handleDownload = (file) => {
        if (!file.file_base64) {
            return addNotification('Немає даних для завантаження файлу. Можливо, бекенд не повертає Base64.', 'error');
        }
        ...
        <div className="file-body column gap-14">
            ...
            {loading ? (
                ...
                <div className="align-center column" style={{ padding: '50px' }}>
                    <div className="text-grey" style={{ color: isDarkTheme ? darkStyles.lightGreyColor : undefined }}>{searchQuery ? 'Файлів не знайдено' : 'Файлів ще немає'}</div>
                </div>
            ) : (
                    ...
                    <div className="file-modal-window" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleAddFile} className="file-form">
                            <label className="file-label">
                                ...
                                <input type="text" placeholder="Назва файлу..." value={title} onChange={(e) => setTitle(e.target.value)} className="file-input" /> 
                            </label>

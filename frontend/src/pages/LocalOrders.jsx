...
import YearMonthFilter from "../components/Order/YearMonthFilter";
// import SidebarFilter from "../components/Order/SidebarFilter";
// import OrdersContent from "./components/Order/OrdersContent";

import "../styles/portal.css";

export default function LocalOrders() {
  const [year, setYear] = useState('2025');       // стан року
  ...
  const handleYearChange = (newYear) => {
    setYear(newYear);
    // Тут можна викликати запит на бекенд для нового року
  };

  const handleMonthChange = (newMonth) => {
    setMonth(newMonth);
    // Тут можна викликати запит на бекенд для нового місяця
  };
  ...
  const calculations = [
    {
      id: 1,
      items: [
        { "ЭтапВыполненияЗаказа": "Новий", "СуммаЗаказа": "1000", "ОплаченоПоЗаказу": "500", "СостояниеЗаказа": "Не подтверждено", "ФактическаяДатаПроизводстваМакс": null, "ФактическаяДатаГотовностиМакс": null, "ДатаРеализации": null },
        { "ЭтапВыполненияЗаказа": "Відмова", "СуммаЗаказа": "500", "ОплаченоПоЗаказу": "0", "СостояниеЗаказа": "Не подтверждено" },
      ]
    },
    {
      id: 2,
      items: [
        { "ЭтапВыполненияЗаказа": "В обробці", "СуммаЗаказа": "2000", "ОплаченоПоЗаказу": "1500", "СостояниеЗаказа": "Подтверждено" }
      ]
    }
  ];

  const handleFilterSelect = (selectedFilter, searchValue) => {
    setFilter(selectedFilter);
    setSearch(searchValue);
    console.log("Selected filter:", selectedFilter, "Search:", searchValue);
  };
  ...
  return (
      ...
      <div className="portal-body row  flex flex-col">
        {/* Sidebar */}
        <div className="w-full bg-gray-50 border-b border-gray-300">
            <YearMonthFilter
              selectedYear={year}
        ...
        <div className="order-body p-2">
          <div className="my-filter">
          <FilterMenu
            calculations={calculations}
            onSelect={(filterId) => setActiveFilter(filterId)}
          />
          </div>
        </div>
      </div>

...
const formatCurrency = (value, unit, i18n) => {
  ...
  // Використовуємо поточну мову для форматування чисел
  const formatter = new Intl.NumberFormat(i18n.language === 'uk' ? "uk-UA" : "en-US", {
    style: "decimal",
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
...
const DocumentRow = React.memo(
  ...
  {firstItem.DocumentType === "КорректировкаДолга" ? (
    <>
      {/* Статичний переклад заголовка + автопереклад динамічного опису */}
      {t('payments.correction')}. <AutoTranslatedText text={firstItem.CorrectionDescription} />
      {firstItem.OrderNumber ? ` (№${firstItem.OrderNumber})` : ""}
    </>
  ) : (
    <>
      {/* Перевіряємо словник: якщо ключа немає, використовуємо автопереклад */}
      { (
        <AutoTranslatedText text={firstItem.DealType || firstItem.DocumentType || "—"} />
      )}
      
      {/* Додаткові деталі для повернень або специфічних номерів */}
      {firstItem.DocumentType === "ВозвратОтПокупателя" && firstItem.DealNumber && (
        `, №${firstItem.DealNumber}`
      )}
    </>
  )}
</td>
          <td>{formatCurrency(docGroup.CumSaldoStart, currency, i18n)}</td>
          <td className={income > 0 ? "text-green" : ""}>
            {income > 0 ? formatCurrency(income, "", i18n) : "—"}
          </td>
          <td className={expense > 0 ? "text-red" : ""}>
            {expense > 0 ? formatCurrency(expense, "", i18n) : "—"}
          </td>
          <td className="text-bold">{formatCurrency(cumSaldo, currency, i18n)}</td>
          <td>
            <span
              className={`channel-badge ${detectPaymentChannel(firstItem)}`}
            >
              {detectPaymentChannel(firstItem) === "bank" && t('payments.channels.bank')}
              {detectPaymentChannel(firstItem) === "cash" && t('payments.channels.cash')}
              {detectPaymentChannel(firstItem) === "order" && t('payments.channels.order_short')}
              {detectPaymentChannel(firstItem) === "none" && "—"}
            </span>
          </td>
          <td colSpan={3} className="td-details">
            {cursorShow ? (
              <span className="expand-btn">
                {isExpanded ? (
                  <>
                    <i className="fa-solid fa-chevron-up" style={{ marginRight: 6 }} />{" "}
                    {t('payments.hide_orders', { count: docGroup.items.length })}
                  </>
                ) : (
                  <>
                    <i className="fa-solid fa-chevron-down" style={{ marginRight: 6 }} />{" "}
                    {t('payments.show_orders', { count: docGroup.items.length })}
                  </>
                )}
              </span>
            ) : (
              <div className="contract-cell">
                <AutoTranslatedText text={firstItem.FinalDogovorName || "—"} />
              </div>
            )}
          </td>
        </tr>
                  ...
                  return (
                    <div key={`${docKey}-${idx}`} className="mini-card clickable-subcard">
                      ...
                      <div className="mini-grid">
                        ...
                        <div>
                          <span className="mini-label">{t('payments.percent')}</span>
                          <span className={item.PaymentStatus < 50 ? "mini-red" : "mini-green"}>
                            {item.PaymentStatus !== null ? `${formatPercent(item.PaymentStatus)} %` : "—"}
                          </span>
                        </div>
          ...
          <td colSpan={11}>
            <div className="date-header">
              ...
              <span className="contracts-text">
                {Object.values(group.initialContracts).map((c, idx) => (
                  <span key={idx} className="contract-badge">
                    <span className="name"><AutoTranslatedText text={c.contractName} />: </span>
                    <span className="value">
                     {formatCurrency(c.initialSaldo, c.currency, i18n)}
                    </span>
                  </span>
                ))}
              </span>
        ...
        <tr className="total-row total-row-separator">
          ...
          <td colSpan={6}>
            <div className="contract-totals">
              {Object.entries(group.contractSummary).map(([name, c], idx) => (
                <div key={idx} className="contract-total-line">
                  <span className="contract-name-bold"><AutoTranslatedText text={name} /></span>:{" "}
                  <span className="text-green">
                    +{formatCurrency(c.income || 0, c.currency, i18n)}
                  </span>{" "}
                  /{" "}
                  <span className="text-red">
                    -{formatCurrency(c.expense || 0, c.currency, i18n)}
                  </span>{" "}
                  /{" "}
                  <span className="text-bold">
                    {formatCurrency(c.lastCumSaldo || 0, c.currency, i18n)}
                  </span>
                </div>
              ))}
            </div>
...
const PaymentStatusV2 = () => {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      link.download = `payment_status_${searchParams.dateFrom}.xlsx`;
      link.click();
      addNotification(t('notifications.excel_success'), "success");
    } catch (_error) {
      addNotification(t('notifications.excel_error'), "warning");
    } finally {
      setExcelLoading(false);
    }
  };
  ...
  const sortedGroups = useMemo(() => {
    ...
    paymentsData.forEach((item) => {
      const date = item.Date?.split("T")[0] || t('payments.unknown_date');
      const docKey = item.RegistrarId
        ? `${item.RegistrarId}_${item.FinalDogovor}_${item.FlowDirection}`
        : `${item.DocumentType}|${item.DocumentNumber}|${date}_${item.FinalDogovor}_${item.FlowDirection}`;
      ...
      dg.lastCumSaldo = toNum(item.CumSaldo);
      const cName = item.FinalDogovorName || t('payments.no_contract');
      if (!group.contractSummary[cName]) {
        group.contractSummary[cName] = {
          income: 0,
          expense: 0,
          lastCumSaldo: 0,
          currency: item.Currency_2 || "грн",
          __t: "",
        };
      }
      const cs = group.contractSummary[cName];
          ...
          <>
            ...
            <div className="table-wrapper">
              <table className="payments-table">
                <thead>
                  <tr>
                    <th>{t('payments.when')}</th>
                    <th>{t('payments.operation')}</th>
                    <th>{t('payments.start_balance')}</th>
                    <th>{t('payments.income')}</th>
                    <th>{t('payments.expense')}</th>
                    <th>{t('payments.balance')}</th>
                    <th>{t('payments.channel')}</th>
                    <th colSpan={3}>{t('payments.details_contract')}</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedGroups.map((g) => (
            ...
            {!paymentsData.length && (
              <div className="text-center p-20">{t('common.no_data')}</div>
            )}
          </>
        ) : (
          <MobilePaymentsView

...
const NotificationDrawer = ({
  ...
  const filteredNotifications = useMemo(() => {
    if (filter === "MESSAGES") {
      return notifications.filter(
        (n) => n.eventType === "NEW_CHAT_MESSAGE" || n.eventType === "NEW_MESSAGE",
      );
    }
    if (filter === "REMINDERS") {
      ...
      <div className={`notification-drawer ${isOpen ? "open" : ""}`}>
        <div className="drawer-header">
          ...
          <div className="icon-together">
            ...
            {permissionStatus === "denied" && (
              <div className="status-blocked" title={t("notifications.push.blocked_title")}>
                <FaExclamationTriangle color="#dc3545" />
              </div>
            )}
            ...
            <Popover
              ...
              content={
                <div className="tg-popover-card">
                  ...
                  <p className="tg-qr-text">{t("notifications.tg.qr_text")}</p>
                  <a href={tgLink} target="_blank" rel="noreferrer" className="tg-direct-link">

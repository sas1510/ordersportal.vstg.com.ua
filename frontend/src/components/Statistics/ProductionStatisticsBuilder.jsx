...
const DEFAULT_LAYOUT = Object.entries(WIDGET_DEFAULTS).map(
  ([type, sizes], index) => ({
...
export default function ProductionStatisticsBuilder({ rawData, dealerData }) {
    ...
    const fetchLayouts = async () => {
      try {
        ...
        if (res.data?.dashboards?.length > 0) {
          ...
          const initial = [
            { id: Date.now(), name: "Мій Дашборд", components: DEFAULT_LAYOUT },
          ];
        ...
        setDashboards([
          { id: Date.now(), name: "Стандарт", components: DEFAULT_LAYOUT },
        ]);
    ...
    const autoSave = async () => {
      ...
      try {
        await axiosInstance.post("/user-dashboard-settings/", {
          dashboards: debouncedDashboards,
        });
  ...
  const updateActiveComponents = (newComponentsOrFn) => {
    setDashboards((prev) =>
      prev.map((d) => {
        ...
        const nextComps =
          typeof newComponentsOrFn === "function"
  ...
  const handleModalSubmit = () => {
    if (modalMode === "create") {
      ...
      setDashboards((prev) =>
        prev.map((d) =>
          d.id === activeDashId ? { ...d, name: modalValue } : d,
        ),
      );
    ...
    ComplexityDonut: {
      ...
      getData: () => {
        ...
        (rawData?.tables?.tech_details || []).forEach((item) => {
          ...
          groups[cat] =
            (groups[cat] || 0) + parseFloat(item.TotalQuantity || 0);
        });
        return {
    ...
    ColorSystemHeatmap: {
      ...
      getData: () => {
        ...
        systems.forEach((s) => {
          ...
          colors.forEach((c) => {
            const cOrders =
              c.OrdersNumber?.split(",").map((n) => n.trim()) || [];
            const inter = sOrders.filter((o) => cOrders.includes(o));
            if (inter.length > 0)
              res.push({
                system: s.ProfileSystem,
  ...
  const addWidget = (type) => {
    ...
    updateActiveComponents((prev) => [
      ...prev,
      { id: newId, type, ...defaultConfig },
    ]);
      ...
      prev.map((c) => {
        ...
        if (axis === "w")
          return {
            ...
            colSpan: Math.max(
              MIN_COL,
              Math.min(GRID_COLUMNS, c.colSpan + delta),
            ),
          };
  ...
  const handleResizeStart = (e, id) => {
    ...
    const colWidth =
      (gridElement.offsetWidth - GAP * (GRID_COLUMNS - 1)) / GRID_COLUMNS;
          ...
          c.id === id
            ? {
                ...
                colSpan: Math.max(
                  ...
                  Math.min(
                    GRID_COLUMNS,
                    initial.colSpan + Math.round(deltaX / (colWidth + GAP)),
                  ),
                ),
                rowSpan: Math.max(
                  MIN_ROW,
                  initial.rowSpan + Math.round(deltaY / GRID_ROW_HEIGHT),
                ),
              }
            : c,
        ),
  ...
  return (
    <div className="builder-page">
      ...
      <div style={{ marginBottom: "14px" }}>
        <div className="dashboard-controls">
          <select
            ...
            value={activeDashId}
            onChange={(e) => setActiveDashId(Number(e.target.value))}
          >
            {dashboards.map((d) => (
          ...
          <button onClick={() => openModal("create")} title="Створити новий">
            <i className="fa fa-plus"></i>
          </button>
          <button onClick={() => openModal("rename")} title="Перейменувати">
        ...
        <button
          className={`palette-toggle ${paletteOpen ? "active" : ""}`}
      ...
      {modalOpen && (
        ...
        >
          <div
            className="builder-modal-window"
          ...
          >
            ...
            <div className="builder-modal-body">
              {modalMode === "delete" ? (
                <div className="builder-modal-text-center">
                  ...
                  <br />
                  <strong className="builder-highlight">
                ...
                <div className="builder-modal-text-center">
                  ...
                  <p className="builder-modal-subtext">
                    Поточне розташування та розміри будуть втрачені
                  </p>
                </div>
              ) : (
            ...
            <div className="builder-modal-footer">
              ...
              >
                <i
                  className={`fa ${modalMode === "delete" ? "fa-trash" : "fa-check"}`}
                ></i>
      ...
      <div className="widget-header">
        ...
        <div className="controls-group">
          ...
          <div className="size-controls">
            <button
              className="size-btn"
              onClick={(e) => onAdjustSize(e, comp.id, "w", 1)}
            >
              +
            </button>
            <button
          ...
          <div className="size-controls">
            <button
              className="size-btn"
              onClick={(e) => onAdjustSize(e, comp.id, "h", 2)}
            >
              +
            </button>
            <button
          ...
          <button
            className="icon icon-cross delete-btn"
            onClick={onDelete}
          ></button>
        </div>
      </div>

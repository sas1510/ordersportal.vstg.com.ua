...
export default function ProductionStatisticsView({ rawData, dealerData }) {
  const chartsData = useMemo(() => {
    ...
    (rawData?.tables?.tech_details || []).forEach((item) => {
      ...
      donutGroups[cat] =
        (donutGroups[cat] || 0) + parseFloat(item.TotalQuantity || 0);
    });
    ...
    const systems = [...(dealerData?.profile_system || [])]
      .sort(
        (a, b) =>
          (parseInt(b.OrdersCount) || 0) - (parseInt(a.OrdersCount) || 0),
      )
      .slice(0, 6);
    ...
    return {
      ...
      donutData: {
        data: Object.entries(donutGroups).map(([name, value]) => ({
          name,
          value,
        })),
      },
    ...
    <div className="modern-dark-view">
      ...
      <main className="modern-content-feed">
        ...
        <div className="feed-section-no-card">
          ...
          <PrefixCategoryDisplayMobile
            prefixData={dealerData?.prefixes || []}
          />
        </div>

        {/* ДИНАМІКА */}
        <div className="modern-feed-card">
          <h3 className="card-title-iconic">
            <i className="fa fa-line-chart"></i> Динаміка продажів
          </h3>
        ...
        <div className="modern-feed-card">
          <h3 className="card-title-iconic">
            <i className="fa fa-windows"></i> Профільні системи
          </h3>
          <div className="m-list-wrapper">
            {chartsData.systems.map((s, _idx) =>
              renderProgressRow(
                s.ProfileSystem,
        ...
        <div className="modern-feed-card">
          <h3 className="card-title-iconic">
            <i className="fa fa-paint-brush"></i> Популярні кольори
          </h3>
          <div className="m-list-wrapper">
            {chartsData.colors.map((c, _idx) =>
              renderProgressRow(
                c.ProfileColor,
        ...
        <div className="modern-feed-card">
          <h3 className="card-title-iconic">
            <i className="fa fa-wrench"></i> Рейтинг фурнітури
          </h3>
          <div className="m-list-wrapper">
            {chartsData.furniture.map((f, _idx) =>
              renderProgressRow(
                f.Name,
        ...
        <div className="modern-feed-card">
          <h3 className="card-title-iconic">
            <i className="fa fa-pie-chart"></i> Категорії виробів
          </h3>
          <div className="chart-container">

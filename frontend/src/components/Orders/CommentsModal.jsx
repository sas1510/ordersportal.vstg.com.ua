...
const CommentsModal = ({ isOpen, onClose, baseTransactionGuid, transactionTypeId, manager }) => {
  ...
  useEffect(() => {
    if (isOpen && baseTransactionGuid && transactionTypeId) {
      ...
      const connectWS = () => {
        ...
        const ws_scheme = window.location.protocol === "https:" ? "wss" : "ws";
        const ws_host = window.location.host;
        socket.current = new WebSocket(`${ws_scheme}://${ws_host}/ws/chat/${chatId}/?token=${token}`);
        ...
        socket.current.onmessage = (e) => {
          ...
          if (data.type === 'chat_message') {
            ...
            if (data.author_id_1c !== currentUser?.user_id_1c) {
                addNotification(`Нове повідомлення: ${data.author}`, "warning");
            }
          }
        };
      };
  ...
  const handleAddComment = () => {
    ...
    if (socket.current?.readyState === WebSocket.OPEN) {
        ...
        // 🔥 Скидаємо висоту поля вводу до початкової після відправки

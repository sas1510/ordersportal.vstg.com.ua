...
//         const firstContract = filteredData[0];
//         setSelectedContract(firstContract.Dogovor_ID);
...
//         } else {
//           setLoadError("У вас немає авансових договорів.");
//         }
//       }
//     } catch (err) {
//       console.error(err);
...
//     const numericValue = Number(value);
//     if (numericValue <= 0) { setPaymentAmount(value); setAmountError("Сума має бути більшою за 0"); return; }
//     const maxAllowed = Math.min(getAvailable(), debt);
//     if (numericValue > maxAllowed) {
//       setAmountError(`Максимальна сума — ${formatCurrency(maxAllowed)}`);
//       setPaymentAmount(maxAllowed.toFixed(2));
//       return;
//     }
//     setPaymentAmount(value);
...
//     </div>
//   ) : loadError ? (
//     <div className="pay-modal-status-centered error-state" style={{ 
//            display: 'flex', 
//            flexDirection: 'row', 
//            alignItems: 'center', 
...
//            padding: '20px 0' 
//          }}>
//       <FaExclamationTriangle size={32} color="#f44336" />
//       <p style={{ marginTop: '10px', textAlign: 'center' }}>{loadError}</p>
//     </div>
//   ) : (
//     <>
//       <div className="pay-currency-info-box">
//         Валюта замовлення: <strong>{orderCurrency}</strong>
//       </div>

//       <p className="pay-debt">
//         Сума до оплати: <strong>{formatCurrency(debt)} {orderCurrency}</strong>
//       </p>

//       <label className="pay-label">
//         <span>Авансовий договір ({orderCurrency}):</span>
//         <div className="custom-dropdown">
//           <div
//             className={`dropdown-selected ${selectedContract ? "active" : ""} ${isSubmitting ? "disabled" : ""}`}
//             onClick={() => !isSubmitting && setDropdownOpen((prev) => !prev)}
//           >
//             <span>{selectedContract ? getSelected()?.Dogovor_Name : "Оберіть договір"}</span>
...
//                   key={c.Dogovor_ID}
//                   className={`dropdown-item ${Number(c.DogovorSum) <= 0 ? "zero-balance" : ""}`}
//                   onClick={() => { setSelectedContract(c.Dogovor_ID); setDropdownOpen(false); }}
//                 >
//                   <div className="item-name">{c.Dogovor_Name}</div>
//                   <div className="item-amount">{formatCurrency(c.DogovorSum)} {c.CurrencyName}</div>
//                 </div>
//               ))}
//             </div>
//           )}
...
//       {selectedContract && (
//         <div className="pay-available">
//           Доступно: <strong className={getAvailable() <= 0 ? "text-red" : ""}>
...
//       <label className="pay-label" style={{ marginTop: '15px' }}>
//         <span>Сума оплати:</span>
//         <input
//           type="number"
//           step="0.01"
...
//         <div className="pay-modal-footer">
//           <button className="pay-btn-cancel" onClick={onClose} disabled={isSubmitting}>
//             <FaTimes size={16} /> Скасувати
//           </button>
//           <button
//             className="pay-btn-confirm"
//             disabled={loading || isSubmitting || !paymentAmount || !!amountError || !selectedContract}
//             onClick={handleConfirm}
//           >
...
export default function PaymentModal({
  ...
  const loadContracts = async () => {
    ...
    try {
      ...
      if (filteredData.length > 0) {
        ...
        if (allData.length > 0) {
          setLoadError(t("payment_modal.error_no_currency", { currency: orderCurrency }));
        } else {
          setLoadError(t("payment_modal.error_no_contracts"));
        }
      }
    } catch (err) {
      setLoadError(t("payment_modal.error_load_failed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAmountChange = (value) => {
    ...
    if (numericValue <= 0) { 
      setPaymentAmount(value); 
      setAmountError(t("payment_modal.error_min_amount")); 
      return; 
    }
    const maxAllowed = Math.min(getAvailable(), debt);
    if (numericValue > maxAllowed) {
      setAmountError(t("payment_modal.error_max_amount", { amount: formatCurrency(maxAllowed) }));
      setPaymentAmount(maxAllowed.toFixed(2));
      return;
    }
    setPaymentAmount(value);
  ...
  return (
    <div className="pay-modal-overlay">
      <div className="pay-modal-window">
        <div className="pay-modal-header">
          <h3>{t("payment_modal.title", { number: order.OrderNumber })}</h3>
          <span className="icon icon-cross pay-close-btn" onClick={!isSubmitting ? onClose : null}></span>
        </div>
        ...
        <div className="pay-modal-body">
          {loading ? (
            <div className="pay-modal-status-centered">
              <div className="loading-spinner-small"></div>
              <span>{t("payment_modal.loading")}</span>
            </div>
          ) : loadError ? (
            <div className="pay-modal-status-centered error-state" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '20px 0' }}>
              <FaExclamationTriangle size={32} color="#f44336" />
              <p style={{ marginTop: '10px', textAlign: 'center' }}>{loadError}</p>
            </div>
          ) : (
            <>
              <div className="pay-currency-info-box">
                {t("payment_modal.order_currency")} <strong>{orderCurrency}</strong>
              </div>

              <p className="pay-debt">
                {t("payment_modal.amount_to_pay")} <strong>{formatCurrency(debt)} {orderCurrency}</strong>
              </p>

              <label className="pay-label">
                <span>{t("payment_modal.advance_contract", { currency: orderCurrency })}</span>
                <div className="custom-dropdown">
                  <div
                    className={`dropdown-selected ${selectedContract ? "active" : ""} ${isSubmitting ? "disabled" : ""}`}
                    onClick={() => !isSubmitting && setDropdownOpen((prev) => !prev)}
                  >
                    <span>{selectedContract ? getSelected()?.Dogovor_Name : t("payment_modal.select_contract")}</span>
                    <span className="dropdown-arrow">{dropdownOpen ? "▲" : "▼"}</span>
                  </div>
              ...
              <label className="pay-label" style={{ marginTop: '15px' }}>
                <span>{t("payment_modal.payment_amount")}</span>
                <input
                  type="number"
                  step="0.01"
                  disabled={isSubmitting || getAvailable() <= 0}
        ...
        <div className="pay-modal-footer">
          <button className="pay-btn-cancel" onClick={onClose} disabled={isSubmitting}>
            <FaTimes size={16} /> {t("payment_modal.btn_cancel")}
          </button>
          <button
            className="pay-btn-confirm"
            disabled={loading || isSubmitting || !paymentAmount || !!amountError || !selectedContract}
          ...
          >
            {isSubmitting ? <FaSpinner className="spinning" /> : <FaCheck />}
            {isSubmitting ? ` ${t("payment_modal.btn_processing")}` : ` ${t("payment_modal.btn_pay")}`}
          </button>
        </div>
      </div>
    </div>

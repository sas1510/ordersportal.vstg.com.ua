      ...
      {sortedData.map((item, _index) => {
        ...
        return (
          ...
          >
            {/* 1. Заголовок */}
            <div
              className={`prefix-card-header ${config.color} text-white text-bold uppercase text-center`}
            >
              {config.label}
            </div>
            ...
            <div className="prefix-card-body">
              ...
              <div className="p-stats-row-main border-bottom-dashed">
                <div className="p-stat-item-large border-right-dashed">
                  ...
                  <div className="p-label text-grey font-size-12 uppercase">
                    Замовлень
                  </div>
                </div>
              ...
              <div className="p-stats-row-secondary">
                ...
                <div className="p-stat-item">
                  <div className="p-value text-info font-size-26 text-bold">
                    {item.AvgCheck
                      ? Math.round(item.AvgCheck).toLocaleString()
                      : 0}
                  </div>
                  <div className="p-label text-grey font-size-12 uppercase">
                    Сер. чек
            ...
            <div className="prefix-card-footer border-top-dashed text-center p-1">
              <small className="text-grey uppercase font-size-10">
                Рекламації по констр.:{" "}
              </small>
              <span
                className={`text-bold font-size-11 ${item.ClaimsPercentage > 5 ? "text-danger" : "text-warning"}`}
              >
                {item.TotalClaims || 0} шт. (
                {item.ClaimsPercentage
                  ? Number(item.ClaimsPercentage).toFixed(2)
                  : "0.00"}
                %)
              </span>
            </div>

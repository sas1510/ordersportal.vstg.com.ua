    ...
    return (
        <div className="modern-mobile-stats">
            {sortedData.map((item) => {
                ...
                return (
                    <div key={prefix} className={`modern-stat-card ${isTotal ? 'is-total' : ''}`}>
                        ...
                        <div className="stat-grid-row">
                            <div className="stat-box green-zone">
                                ...
                                <span className="val">{Math.round(item.AvgPricePerConstruction || 0).toLocaleString()}</span>
                                <span className="lbl">Сер. ціна конструкції</span>
                            </div>
                            <div className="stat-box red-zone">
                                <i className="fa fa-calculator"></i>
                                <span className="val">{Math.round(item.AvgCheck || 0).toLocaleString()}</span>
                        ...
                        <div className="comment-zone">
                            <div className="comment-header">
                                <span><i className="fa fa-exclamation-triangle"></i> Рекламації по конструкціях:</span>
                                <span className={(item.TotalClaims > 0) ? 'text-warn' : ''}>
                                    {item.TotalClaims || 0} шт. ({Number(item.ClaimsPercentage || 0).toFixed(2)}%)
                                </span>
                            </div>
                        </div>
                    </div>

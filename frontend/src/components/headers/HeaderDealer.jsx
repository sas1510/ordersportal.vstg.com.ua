...
//         `${ws_scheme}://${ws_host}/ws/notifications/?token=${token}`,
//       );
...
//           addNotification(
//             `🔔 ${payload.author_name || ""}: ${newEntry.message}`,
//             "info",
//             6000,
//           );
//         }
...
//         if (!isCleanup) {
//           console.log(
//             `Notification WS closed (code ${e.code}). Reconnecting in 5s...`,
//           );
...
//             <li>
//               <button className="menu-link" onClick={toggleFinanceMenu}>
//                 <i className="icon icon-coin-dollar"></i>Фінанси ▾
//               </button>
//               {showFinanceMenu && (
//                 <ul className="submenu">
//                   {FINANCE_SUBMENU.map((item) => (
...
//                     <Link
//                       to="/emergency-contacts"
//                       className="menu-link menu-link--sos"
//                       onClick={() => setProfileOpen(false)}
//                     >
//                       Гаряча лінія
//                     </Link>
...
//                 >
//                   <i
//                     className="fa fa-bell material-icons"
//                     style={{
//                       color: theme === "light" ? "#f4ffaf" : "#ffc107",
//                       fontSize: "18px",
//                       fontStyle: "normal",
...
useEffect(() => {
  const handleClickOutside = (event) => {
    // Закриваємо Фінанси, якщо клік зовні
    if (financeRef.current && !financeRef.current.contains(event.target)) {
      setShowFinanceMenu(false);
    }
    // Закриваємо Профіль, якщо клік зовні (ДОДАНО)
    if (profileRef.current && !profileRef.current.contains(event.target)) {
      setProfileOpen(false);
    }
  ...
  <div className={`w-full max-w-[1334px] h-[70px] bg-white flex items-center shadow-md relative 
    ...
    {!isMobile ? (
      <>
        {/* Навігація - Додаємо flex-grow, щоб вона зайняла весь вільний простір */}
  ...
  >
    ...
    {profileOpen && (
      ...
      {/* Drawer зі сповіщеннями (зберігаємо логіку) */}
      <NotificationDrawer
        isOpen={isNotificationOpen}
        notifications={notifications}
        setNotifications={setNotifications}

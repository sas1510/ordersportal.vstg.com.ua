$(document).ready(function () {
  ...
  $(".fullscreen-btn").on("click", function (event) {
    ...
    const link =
      $(this).closest(".item").attr("data-target") +
      "?key=regional-5ABvgGQPOGy5SgJAVkV6K5LcAkFjx2Zxeandvmx6Zu7sfw4oQTRKTfz";
  ...
  $(".item").on("click", function (event) {
    ...
    if (target) {
      ...
      $(".window-title").html(
        '<span class="icon-full-moon text-info" style="margin: 10px; bottom: -2px; position: relative;"></span><span>' +
          $(this).find(".item-title").text() +
          "</span>",
      );
      document.title =
        "Портал аналітики - " + $(this).find(".item-title").text();
    }
  });

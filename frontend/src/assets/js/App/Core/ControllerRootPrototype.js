  ...
  constructor(data = {}, parent = this, root = this) {
    ...
    if (Object.keys(data).length > 0 && data.hasOwnProperty("items")) {
      for (let key in data.items) {
        try {
          eval(
            `this.items[key] = new ${data.items[key].class}(data.items[key], this, root)`,
          );
        } catch (e) {
          console.error(
            (this.className =
      ...
      for (let key in obj) {
        if (typeof obj[key] !== "object") {
          if (result.hasOwnProperty(key)) {
            result[key] += isNaN(parseFloat(obj[key]))
              ? 0
              : parseFloat(obj[key]);
          } else {
            result[key] = isNaN(parseFloat(obj[key]))
  ...
  filterByInternals(source = {}, filter = {}) {
    ...
    for (let key in source) {
      ...
      for (let key in filter) {
        ...
        if (
          sourceItem.internals.hasOwnProperty(key) &&
          sourceItem.internals[key] == filterValue
        ) {
          matchCount++;
        }
      }

class ViewRootPrototype {
  ...
  tableRowTmpl = {
    ...
    timeAlarm: {
      ...
      isFalse:
        '<span class="icon-clock3 text-danger" title="Протермінований"></span>',
      isNull: "",
      isNil: "",
      isEmpty: "",
      style: "table-ico",
    ...
    isРrovided: {
      ...
      type: "bool",
      isTrue:
        '<span class="icon-checkmark text-success font-16" title="Забезпечений"></span>',
      isFalse:
        '<span class="icon-cancel text-danger" title="Не забезпечений"></span>',
      isNull: "",
      isNil: "",
      isEmpty: "",
      style: "table-ico",
    ...
    OrderStatus: {
      ...
      inProduction:
        '<span class="icon-cogs text-info" title="У виробництві"></span>',
      inBuffer:
        '<span class="icon-layers2 text-grey" title="Не запланований"></span>',
      Planned:
        '<span class="icon-clipboard text-warning" title="Запланований у виробництво"></span>',
      notConfirmed:
        '<span class="icon-warning3 text-danger" title="Не підтверджений"></span>',
      isFalse: "",
      isNull: "",
      isNil: "",
      isEmpty: "",
  ...
  ifZerro(value) {
    return value
      ? value
      : `<span class="icon-dots-three-horizontal text-grey"></span>`;
  }
  ...
  numToUAMoneyFormat(num, currencyName = "грн") {
    ...
    const currency =
      currencyName in currencies ? currencies[currencyName] : "UAH";
  ...
  eventHandlerClickTrigger(_this = this.parent) {
    ...
    $(document).on("click", ".clickTrigger", function (event) {
      ...
      modalID = _this.view.renderModal(
        $(this).attr("title"),
        _this.internals.name ?? "",
        modalColor,
        modalWidth,
      );
  ...
  renderTableHeader(template) {
    ...
    for (let key in template) {
      if (
        template[key].type !== "hidden" &&
        template[key].type !== "rowStatus"
      ) {
  ...
  renderTable(
    data = {},
    tmpl = this.tableRowTmpl,
    tableID,
    beforeContent = `<div class="modal-before-wrapper"></div>`,
    afterContent = `<div class="modal-after-wrapper"></div>`,
  ) {
    let rowChain = "";
    for (let key in data) {
